...
const Products = () => {
  ...
  const filtered = products.filter((p) => {
    const matchesCategory = active === "All" || p.category.toLowerCase() === active.toLowerCase();
          ...
          <div className="hidden lg:block space-y-2 pt-8 border-t border-white/5">
            ...
            {dynamicCategories.map((cat) => (
              <button
                ...
                className={`block w-full text-left text-lg font-display font-medium transition-all duration-300 ${active.toLowerCase() === cat.toLowerCase() ? "text-gold translate-x-2" : "text-gray-500 hover:text-white hover:translate-x-1"
                  }`}
              >
                {cat}
              </button>
            ...
            <div className="flex gap-4 min-w-max">
              ...
              {dynamicCategories.map((cat) => (
                <button
                  key={cat}
                  onClick={() => setActive(cat)}
                  className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-colors ${active.toLowerCase() === cat.toLowerCase() ? "bg-gold text-black" : "bg-white/5 text-white border border-white/10"
                    }`}
                >
                  {cat}
                </button>
